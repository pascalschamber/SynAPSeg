"""
code is from https://github.com/labsyspharm/ome-tiff-pyramid-tools
with slight modifications to run without using argparse
"""
...
def preduce(coords, img_in, img_out, is_mask): 
    ...
    if is_mask:
        ...
        tile = skimage.transform.downscale_local_mean(tile, (2, 2)) # does this affect image intensity of the highest resolution image?
...
def main(in_paths, out_path, px_size=1, is_mask=False):
    ...
    xml = construct_xml(
        os.path.basename(out_path), shapes, num_channels, ome_dtype, px_size
    )
