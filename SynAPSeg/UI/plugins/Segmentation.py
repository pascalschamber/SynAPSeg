...
class MainApp(BaseApp):
    ...
    def _run(self):
        ...
        if self._running_in_test_mode:
            error_msg = self.inject_test_mode_params(built_config)
            if error_msg:
    ...
    def get_built_config(self) -> dict:
        ...
        print('\n\n in built config validated config:')
    ...
    def handle_run_worker_complete(self, result):
        ...
        Handle the output from the run worker thread.
            - When run in test mode, shows results in a napari window.
            - The result is expected to be a tuple of (arr, predictions, ex_md).
        ...
        arr, predictions, ex_md = result

        # show results in a napari window 
        ########################################################


        
        # coerce prediction data to napari viewer format
        #     - img dict keys are file STEMS, values are arrays
        ########################################################################
        ...
        _shapes = {k: arr.shape for k,arr in predictions.items()}
        _fmts = {k:STANDARD_FORMAT for k,arr in predictions.items()}
        ex_md['data_metadata'] = {
            'data_shapes': _shapes, 'data_formats': _fmts
        }
    ...
    def inject_test_mode_params(self, validated_config):
        """if run is being executed in test params mode, inject required args"""
        
        file_paths, error_msg = self.fetch_image_filepath_selection(validated_config)
        if error_msg:
            return error_msg
    ...
    def fetch_image_filepath_selection(self, config:dict):
        ...
        def _parse_available_filepaths(config) -> tuple[list[str], str]:
            ...
            try:
                ...
                errormsg = f"Error parsing filepaths: {e}"
                return [], errormsg
        ...
        selected_filepaths = _display_filepath_selection_dialog(self,available_filepaths)
    ...
    def make_project_params_layout(self):
        ...
        self.load_project_seg_config_button.clicked.connect(self.load_seg_params_log)
    ...
    def save_project_seg_config(self): # TODO
        ...
        if seg_params and log_path and os.path.exists(log_path) and log_config:
            ...
            #     pass
            #     # yaml.dump(log_config, file, default_flow_style=False)
    ...
    def load_seg_params_log(self):
        ...
        if PROJECT_NAME and seg_config_path:             
            try: 
                ...
                print(f'segmentation_config_log_path not set\n{e}')

        else:
            print('segmentation_config_log_path not set')
    ...
    def set_project_seg_config(self, interp):
        ...
        self.set_seg_config_status.setText("load_project_seg_config successful.")
